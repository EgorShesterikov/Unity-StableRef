        ...
        private static void ScanSerializedObject(SerializedObject so, Node parent, UnityEngine.Object pingTarget)
        {
            ...
            while (iter.Next(enter))
            {
                ...
                if (iter.isArray
                    ...
                    && StableRefPropertyUtils.HasStableRefSelectorAttribute(iter))
                {
                    string label = StableRefEditorUtility.BuildFieldDisplayPath(so, iter.propertyPath);
                    var node = BuildListNode(iter, pingTarget, label);
